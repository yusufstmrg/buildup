    ...
    <>
      <aside className="w-64 bg-brand-surface border-r border-brand-border flex flex-col h-full shrink-0 select-none">
        ...
        <div className="px-4 pt-4 pb-2">
          <Link
            to="/"
            className="flex items-center justify-between px-3 py-2 rounded-lg bg-brand-navy border border-brand-border hover:border-brand-gold/40 text-xs font-semibold text-slate-300 hover:text-white transition-all group"
          >
        ...
        <div className="flex-1 px-3 py-2 space-y-1 overflow-y-auto">
          <div className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider px-3 py-2">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <Icon className={`w-4 h-4 ${isActive ? 'text-brand-gold' : item.highlight ? 'text-brand-gold' : 'text-slate-400 group-hover:text-slate-200'}`} />
                  <span>{item.name}</span>
                </div>
                {item.badge && (
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${
                    item.badge.includes('Alert') 
                      ...
                      : 'bg-brand-navy border border-brand-border text-slate-400'
                  }`}>
                    {item.badge}
                  </span>
                )}
        ...
        <div className="p-3 border-t border-brand-border">
          <div className="bg-brand-navy rounded-xl p-3 border border-brand-border text-xs">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-slate-400 font-medium">BuildUp Score™</span>
        ...
        <div className="p-3 bg-brand-deep/50 border-t border-brand-border">
          <div className="rounded-xl p-3 bg-gradient-to-b from-brand-card/60 to-brand-navy border border-brand-border">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] font-bold text-white uppercase tracking-wider">Human Expert Escalation</span>
            </div>
            ...
            {!expertSubmitted ? (
              <div className="space-y-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-white">Escalate to BuildUp Expert Partner</h3>
                    <p className="text-xs text-slate-400">Section 12: Certified Human Expert Network</p>
                  </div>
                </div>
                ...
                <div className="space-y-3 text-xs">
                  <div>
                    <label className="block text-slate-300 font-semibold mb-1">Matter Category</label>
                    <select className="w-full bg-brand-navy border border-brand-border rounded-lg p-2.5 text-white">
              ...
              <div className="text-center py-6 space-y-3">
                ...
                <h4 className="text-lg font-bold text-white">Escalation Request Dispatched</h4>
