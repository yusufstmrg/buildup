...
export function Sidebar() {
  ...
  const navigation = [
    {
      name: t("navCommandCenter") || "Command Center",
      href: "/app",
      icon: LayoutDashboard,
    },
    {
      name: t("navIntelligence") || "BuildUp Intelligence™",
      href: "/intelligence",
      icon: BrainCircuit,
      badge: "Active Graph",
    },
    {
      name: t("navDiagnostics") || "Diagnostics & X-Ray™",
      href: "/diagnostics",
      icon: ActivitySquare,
    },
    {
      name: t("navWorkforce") || "AI Workforce",
      href: "/workforce",
      icon: Users,
      badge: "12 Roles",
    },
    {
      name: t("navBusinessOS") || "Business OS™",
      href: "/business-os",
      icon: Settings,
    },
    {
      name: t("navPlanner") || "Strategic Planner",
      href: "/planner",
      icon: TrendingUp,
    },
    {
      name: t("navControlEngine") || "Internal Control Engine",
      href: "/control",
      icon: ShieldAlert,
      badge: "3 Alerts",
    },
    {
      name:
        t("navMonetization") ||
        (language === "id" ? "Paket & Tagihan" : "Billing & Plans"),
      href: "/monetization",
      icon: Coins,
      highlight: true,
    },
    {
      name: language === "id" ? "Kemitraan & Afiliasi" : "Referral & Affiliate",
      href: "/affiliate",
      icon: Share2,
    },
    ];
      ...
      <aside className="w-64 bg-brand-surface border-r border-brand-border flex flex-col h-full shrink-0 select-none">
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                to={item.href}
                className={`group flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold transition-all ${isActive ? "bg-brand-gold/15 text-brand-gold border border-brand-gold/30 shadow-gold-sm" : item.highlight ? "text-brand-gold hover:bg-brand-gold/10" : "text-brand-textMuted hover:text-brand-textMain hover:bg-brand-card/60"}`}
              >
                {" "}
                <div className="flex items-center gap-2.5">
                  {" "}
                  <Icon
                    className={`w-4 h-4 ${isActive ? "text-brand-gold" : item.highlight ? "text-brand-gold" : "text-brand-textMuted group-hover:text-brand-textMain"}`}
                  />{" "}
                  <span>{item.name}</span>{" "}
                </div>{" "}
                {item.badge && (
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${item.badge.includes("Alert") ? "bg-red-500/20 text-red-400" : "bg-brand-navy border border-brand-border text-brand-textMuted"}`}
                  >
                    {" "}
                    {item.badge}{" "}
                  </span>
        ...
        <div className="p-3 border-t border-brand-border">
          ...
          <div className="bg-brand-navy rounded-xl p-3 border border-brand-border text-xs">
            ...
            >
              {" "}
              <Sparkles className="w-3 h-3" /> Retake Health Check{" "}
            </button>{" "}
          </div>{" "}
        </div>{" "}
        ...
        <div className="p-3 bg-brand-deep/50 border-t border-brand-border">
          ...
          <div className="rounded-xl p-3 bg-gradient-to-b from-brand-card/60 to-brand-navy border border-brand-border">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] font-bold text-brand-textMain uppercase tracking-wider">
                Human Expert Escalation
              </span>{" "}
            </div>{" "}
            <p className="text-[10px] text-brand-textMuted mb-2 leading-relaxed">
              {" "}
              Connect to licensed Indonesian KAP Auditors, Tax Advisors, and
              Corporate Counsel.{" "}
            </p>{" "}
            <button
            ...
            {!expertSubmitted ? (
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <h3 className="text-base font-bold text-brand-textMain">
                      Escalate to BuildUp Expert Partner
                    </h3>{" "}
                    <p className="text-xs text-brand-textMuted">
                      Section 12: Certified Human Expert Network
                    </p>{" "}
                  </div>{" "}
                </div>{" "}
                <p className="text-xs text-brand-textMuted leading-relaxed">
                  {" "}
                  BuildUp pairs autonomous AI execution with top-tier human
                  professionals when material judgment or statutory sign-off is
                  required.{" "}
                </p>{" "}
                <div className="space-y-3 text-xs">
                  ...
                  <div>
                    {" "}
                    <label className="block text-brand-textMuted font-semibold mb-1">
                      Matter Category
                    </label>{" "}
                    <select className="w-full bg-brand-navy border border-brand-border rounded-lg p-2.5 text-brand-textMain">
                      {" "}
                      <option>
                        KAP Audit / Financial Statement Sign-Off
                      </option>{" "}
                      <option>
                        Indonesian Tax Advisory (SP2DK / Tax Audit Defense)
                      </option>{" "}
                      <option>Corporate Legal & Cross-Border Contracts</option>{" "}
                      <option>
                        Supply Chain & Strategic Sourcing Negotiation
                      </option>{" "}
                      <option>
                  ...
                  <div>
                    ...
                    <select className="w-full bg-brand-navy border border-brand-border rounded-lg p-2.5 text-brand-textMain">
                      ...
                      <option>
                        Immediate / Audit Defense (Same-Day Response)
                      </option>{" "}
                    </select>{" "}
                  </div>{" "}
                </div>{" "}
              ...
              <div className="text-center py-6 space-y-3">
                ...
                <h4 className="text-lg font-bold text-brand-textMain">
                  Escalation Request Dispatched
                </h4>{" "}
                <p className="text-xs text-brand-textMuted max-w-sm mx-auto">
                  {" "}
                  A certified BuildUp Partner (Senior Advisory Partner) has
                  received the contextual dossier and Decision Object audit
                  trail. Expect direct contact within 2 hours.{" "}
                </p>{" "}
                <button
                  onClick={() => {
                    setShowExpertModal(false);
