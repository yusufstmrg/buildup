        ...
        <div className="bg-gradient-to-r from-amber-600 via-brand-gold to-amber-600 text-slate-950 px-4 py-1.5 flex items-center justify-between text-xs font-bold shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2.5 py-0.5 bg-slate-950 hover:bg-slate-900 text-brand-gold hover:text-white rounded-lg text-[11px] font-extrabold transition-colors"
            >
        ...
        <div className="flex-1 flex items-center max-w-xl">
          <div className="relative w-full">
            ...
            <input 
              ...
              className="w-full bg-brand-navy border border-brand-border rounded-xl pl-10 pr-4 py-2 text-xs text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-brand-gold/60 focus:ring-1 focus:ring-brand-gold/50 transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 ml-4">
          ...
          <div className="hidden sm:flex items-center bg-brand-navy border border-brand-border rounded-lg p-0.5 text-xs">
            <button 
              ...
              className={`px-2 py-0.5 rounded font-bold text-[11px] transition-colors ${currency === 'IDR' ? 'bg-brand-gold text-brand-deep' : 'text-slate-400 hover:text-white'}`}
            >
              IDR
            </button>
            <button 
              onClick={() => setCurrency('USD')}
              className={`px-2 py-0.5 rounded font-bold text-[11px] transition-colors ${currency === 'USD' ? 'bg-brand-gold text-brand-deep' : 'text-slate-400 hover:text-white'}`}
            >
              USD
            </button>
          </div>
          ...
          <Link
            to="/"
            className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand-navy hover:bg-brand-card text-slate-300 hover:text-brand-gold border border-brand-border text-xs font-semibold transition-colors"
          >
          ...
          <div className="relative">
            <button 
              ...
              className="relative p-2 text-slate-400 hover:text-white hover:bg-brand-navy rounded-lg transition-colors"
            >
                ...
                <div className="space-y-1.5">
                  {notifications.map((n, i) => (
                    <div key={i} className={`p-2 rounded-lg text-xs ${n.unread ? 'bg-brand-navy border border-brand-border/80' : 'bg-transparent'}`}>
                      ...
                      <div className="text-[11px] text-slate-400 leading-snug mt-0.5">{n.desc}</div>
          ...
          <div className="relative" ref={userMenuRef}>
            ...
            {/* User Dropdown Menu */}
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl p-3 z-50 animate-fade-in backdrop-blur-xl">
                <div className="pb-3 mb-2 border-b border-slate-800">
                  <p className="text-xs font-extrabold text-white truncate">{user?.companyName || 'PT Global Distribusi Nusantara'}</p>
                  <p className="text-[11px] text-slate-400 truncate mt-0.5">{user?.email || 'director@nusantara-group.co.id'}</p>
