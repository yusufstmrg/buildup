...
interface BuildUpContextType {
  ...
  processPaymentSuccess: (
    planId: UserSubscriptionInfo["planId"],
    planName: string,
  ) => void;
...
const initialDimensions: HealthDimension[] = [
  {
    ...
    benchmark: 0,
    status: "Pending",
    findings: "Belum ada data mendetail",
    bottleneck: "Menunggu integrasi sistem",
  },
  {
    name: "Sales & Growth",
    score: 0,
...
export function BuildUpProvider({ children }: { children: React.ReactNode }) {
  ...
  const [language, setLanguageState] = useState<Language>(() => {
    ...
    return saved && ["id", "en", "zh", "ja", "ar"].includes(saved)
      ? saved
      : "id";
  });
  ...
  const [hasCompletedHealthCheck, setHasCompletedHealthCheck] =
    useState<boolean>(() => {
      return !!localStorage.getItem("bu_score");
    });
  const [hasPendingDiagnostic, setHasPendingDiagnostic] =
    useState<boolean>(false);
  const [dimensions, setDimensions] =
    useState<HealthDimension[]>(initialDimensions);
  const [decisionObjects, setDecisionObjects] =
    useState<DecisionObject[]>(initialDecisions);
  ...
  const [totalAnnualLeakageIdr, setTotalAnnualLeakageIdr] = useState<number>(
    () => (user?.isSandbox ? 1450000000 : 0),
  );
  const [totalAnnualLeakageUsd, setTotalAnnualLeakageUsd] = useState<number>(
    () => (user?.isSandbox ? 96000 : 0),
  );

  const setGlobalHealthScore = (
    score: number,
    findings: string[],
    rec: string,
  ) => {
  ...
  const saveDiagnosticToSession = (
    score: number,
    findings: string[],
    rec: string,
  ) => {
    sessionStorage.setItem(
      "bu_pending_diagnostic",
      JSON.stringify({
        score,
        findings,
        rec,
    ...
    setDimensions((prev) =>
      prev.map((dim, idx) => {
        ...
        const dimScore = Math.min(
          100,
          Math.max(35, val * 20 - Math.floor(Math.random() * 8)),
        );
        return {
          ...dim,
          score: dimScore,
          status:
            dimScore < 65 ? "Critical" : dimScore < 80 ? "Warning" : "Healthy",
        };
      }),
    );
  };
  ...
  const processPaymentSuccess = (
    ...
    setSubscription((prev) => ({
      ...
      metrics: {
        aiInsights: {
          ...prev.metrics.aiInsights,
          limit: planId === "health-check" ? 10 : 100,
        },
        integrations: {
          ...prev.metrics.integrations,
          limit: planId === "health-check" ? 3 : 10,
        },
        workflows: {
          ...prev.metrics.workflows,
          limit: planId === "health-check" ? 1 : 25,
        },
      },
    }));
  };
