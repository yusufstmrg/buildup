  ...
  return (
    <div className="space-y-6">
        ...
        <div className="border border-brand-gold/50 bg-gradient-to-r from-brand-gold/10 via-brand-gold/5 to-transparent rounded-2xl p-5 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1">
              {" "}
              <div className="text-sm font-black text-brand-gold mb-1">
                Laporan Diagnosis Anda Sudah Tersimpan!
              </div>{" "}
              <p className="text-xs text-brand-textMuted">
                {" "}
                Selamat datang di BuildUp Command Center. Hasil audit bisnis
                Anda sudah terhubung ke dashboard — lihat BuildUp Score™ dan
                temuan kritis di bawah.{" "}
              </p>{" "}
            </div>{" "}
          ...
          {criticalSignals.length > 0 && (
            <div className="mt-3 pt-3 border-t border-brand-gold/20 space-y-1">
              ...
              {criticalSignals.slice(0, 2).map((signal, i) => (
                <p
                  key={i}
                  className="text-xs text-brand-textMuted flex items-start gap-2"
                >
                  {" "}
              ...
              {criticalSignals.length > 2 && (
                <p className="text-[10px] text-brand-gold font-semibold">
                  +{criticalSignals.length - 2} temuan lainnya tersimpan di
                  laporan Anda
                </p>
              )}{" "}
            </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            {" "}
            <h1 className="text-2xl font-black text-brand-textMain tracking-tight">
          ...
          <p className="text-xs text-brand-textMuted mt-1">
            {" "}
            Real-time cross-functional synthesis across Finance, Sales,
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {" "}
            <Sparkles className="w-4 h-4" /> <span>Run Health Check™</span>{" "}
          </button>{" "}
          <Link
            to="/planner"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="xl:col-span-3 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-brand-surface border border-brand-border hover:border-brand-gold/40 rounded-xl p-5 relative overflow-hidden transition-all group">
              {" "}
              <div className="text-xs font-bold text-brand-textMuted uppercase tracking-wider mb-1">
                BuildUp Score™
              </div>{" "}
              <div className="text-3xl font-black text-gold-gradient tracking-tight mb-2">
                {" "}
                {hasCompletedHealthCheck ? overallScore : "--"}
                <span className="text-sm font-normal text-brand-textMuted">
                  /100
                </span>{" "}
              </div>{" "}
            ...
            <div className="bg-brand-surface border border-brand-border hover:border-brand-gold/40 rounded-xl p-5 relative overflow-hidden transition-all group">
              ...
              <div className="text-2xl font-black text-brand-textMain tracking-tight mb-2">
                ...
                {hasCompletedHealthCheck
                  ? formatMoney(
                      totalAnnualLeakageIdr * 1.2,
                      totalAnnualLeakageUsd * 1.2,
                    )
                  : formatMoney(0, 0)}{" "}
              </div>{" "}
              <div className="flex items-center justify-between text-xs">
            ...
            <div className="bg-brand-surface border border-brand-border hover:border-brand-gold/40 rounded-xl p-5 relative overflow-hidden transition-all group">
              ...
              <div className="text-2xl font-black text-brand-textMain tracking-tight mb-2">
                {" "}
                {hasCompletedHealthCheck ? "18 / 18" : "0 / 18"}{" "}
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-emerald-400 font-semibold flex items-center gap-1">
                  ...
                  {hasCompletedHealthCheck ? "99.4% SLA" : "--% SLA"}{" "}
                </span>{" "}
                <Link
                  to="/workforce"
                  className="text-[11px] text-brand-gold hover:underline font-bold"
            ...
            <div className="bg-brand-surface border border-brand-border hover:border-brand-gold/40 rounded-xl p-5 relative overflow-hidden transition-all group">
              ...
              <div className="text-2xl font-black text-emerald-400 tracking-tight mb-2">
                {" "}
                {hasCompletedHealthCheck
                  ? formatMoney(totalAnnualLeakageIdr, totalAnnualLeakageUsd)
                  : formatMoney(0, 0)}{" "}
              </div>{" "}
              <div className="flex items-center justify-between text-xs">
                {" "}
                <span className="text-yellow-400 font-semibold flex items-center gap-1">
                  ...
                  {hasCompletedHealthCheck
                    ? `${criticalSignals.length || 0} Bottlenecks`
                    : "0 Bottlenecks"}{" "}
                </span>{" "}
                <Link
                  to="/diagnostics"
          ...
          <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-4 pb-4 border-b border-brand-border">
              ...
              <div>
                ...
                <p className="text-xs text-brand-textMuted mt-0.5">
                  {" "}
                  Click any node to reveal second-order financial & operational
                  ripple effects.{" "}
                </p>{" "}
              </div>{" "}
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${isSelected ? "bg-brand-gold/15 border-brand-gold shadow-gold-sm" : n.status === "alert" ? "bg-red-500/10 border-red-500/40 text-red-300 hover:border-red-400" : n.status === "warning" ? "bg-yellow-500/10 border-yellow-500/40 text-yellow-300 hover:border-yellow-400" : "bg-brand-card/40 border-brand-border text-brand-textMuted hover:border-brand-borderLight"}`}
                  >
                    {" "}
                    <div className="text-[11px] font-bold text-brand-textMain truncate">
                      {n.label}
                    </div>{" "}
            ...
            <div className="mt-4 p-4 rounded-xl bg-brand-navy border border-brand-border flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-brand-textMuted mt-0.5 font-italic">
                  Sistem belum menerima cukup data untuk men-generate causal
                  trace untuk node ini.{" "}
                </p>{" "}
              </div>{" "}
              <Link
                to="/intelligence"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 bg-brand-surface border border-brand-border rounded-2xl p-6 flex flex-col">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div>
                  {" "}
                  <h2 className="text-base font-bold text-brand-textMain">
                    Revenue, Margin & Cash Trajectory
                  </h2>{" "}
                  <p className="text-xs text-brand-textMuted">
                    Weekly trajectory with BuildUp operational intervention
                  </p>{" "}
                </div>{" "}
                ...
                {chartData.length > 0 ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-brand-textMuted border-2 border-dashed border-brand-border rounded-xl">
                    ...
                    <p className="text-xs max-w-[250px] text-center mt-1">
                      Sistem belum menerima aliran data historis Anda. Hubungkan
                      ERP Anda untuk melihat statistik.
                    </p>{" "}
                  </div>
                )}{" "}
              </div>{" "}
            ...
            <div className="lg:col-span-5">
              {" "}
              <PremiumLock
                featureName="Autonomous Decision Engine"
                requiredPlan="growth"
                    ...
                    <div className="space-y-3">
                      ...
                      {decisionObjects.length > 0 ? (
                        decisionObjects.slice(0, 2).map((dec) => (
                          <div
                            key={dec.id}
                            className="p-4 rounded-xl bg-brand-navy border border-brand-border space-y-2.5"
                          >
                            {" "}
                            <div className="flex items-center justify-between text-xs">
                              {" "}
                        ...
                        <div className="py-8 flex flex-col items-center justify-center text-brand-textMuted text-center">
                          ...
                          <p className="text-xs font-semibold">
                            Belum Ada Keputusan Tertunda
                          </p>{" "}
                          <p className="text-[10px] mt-1 max-w-[200px]">
                            AI Gateway sedang memantau operasi Anda. Keputusan
                            otomatis akan muncul di sini.
                          </p>{" "}
                        </div>
                      )}{" "}
                    </div>{" "}
        ...
        {/* Close Main Dashboard Column */} {/* Upgrade Center Column */}{" "}
