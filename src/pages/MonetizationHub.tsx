    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-black text-white tracking-tight">Commercial & Monetization Engine</h1>
        ...
        <div className="flex items-center bg-brand-surface border border-brand-border rounded-xl p-1 text-xs">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg font-bold transition-all ${currency === 'IDR' ? 'bg-brand-gold text-brand-deep shadow-sm' : 'text-slate-400 hover:text-white'}`}
          >
            IDR (Rupiah)
          </button>
          <button
            onClick={() => setCurrency('USD')}
            className={`px-3 py-1.5 rounded-lg font-bold transition-all ${currency === 'USD' ? 'bg-brand-gold text-brand-deep shadow-sm' : 'text-slate-400 hover:text-white'}`}
          >
            USD ($)
          </button>
        </div>
      ...
      <div className="bg-gradient-to-r from-brand-card to-brand-navy border border-brand-gold/40 rounded-2xl p-6 shadow-2xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="text-right shrink-0">
            <div className="text-xs text-slate-400 font-semibold">Attributed ROI</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-bold text-white">Commercial Tier Packaging</h2>
          <span className="text-xs text-slate-400">Section 16.1 Product Pricing Architecture</span>
        </div>
                ...
                <div className="mt-8 pt-4 border-t border-brand-border">
                  <button
                    ...
                    className={`w-full py-3 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${
                      isCurrent
                        ? 'bg-brand-navy border border-brand-border text-slate-400 cursor-default'
                        : p.highlight
                        ...
                        : 'bg-brand-card hover:bg-brand-border text-white border border-brand-border'
                    }`}
                  >
            ...
            {!checkoutSuccess ? (
              <div className="space-y-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-white">Commercial Order & Invoice Generation</h3>
                    <p className="text-xs text-slate-400">Plan Selected: <strong>{selectedPlanToUpgrade}</strong></p>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-xl bg-brand-navy border border-brand-border text-xs space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white font-semibold">PT Nusantara Jaya Abadi</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Billing Cycle:</span>
                    <span className="text-white font-semibold">Contractual / Monthly</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-3 bg-brand-card hover:bg-brand-border text-slate-300 font-bold text-xs rounded-xl"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="text-center py-6 space-y-3">
                ...
                <button
                  onClick={() => setShowCheckoutModal(false)}
                  className="mt-4 px-6 py-2 bg-brand-navy border border-brand-border text-white text-xs font-semibold rounded-lg"
                >
                  Return to Hub
                </button>
              </div>
